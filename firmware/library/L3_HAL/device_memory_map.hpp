...
Register_t
{
  ...
  bool operator=(Int value)
  {
    ...
    bool status = write(address, sizeof(Int), reinterpret_cast<uint8_t*>(&local));
    return status;
  }
  MEMORY_OPERATION(|);
  MEMORY_OPERATION(&);
...
template <class DeviceProtocol, device::Endian endianess,
          template <device::Endian endian, WriteFnt write, ReadFnt read> class MemoryMap>
class Device
{
 private:
 ...
 public:
  ...
  MemoryMap<endianess, Device::Write<DeviceProtocol>, Device::Read<DeviceProtocol>> &
      memory;
 ...
 public:
  ...
  static bool Write(intptr_t address, size_t size, uint8_t * target)
  {
    ...
    I2cInterface::Status status =
        i2c->Write(kDeviceAddress, payload, size + 1);
